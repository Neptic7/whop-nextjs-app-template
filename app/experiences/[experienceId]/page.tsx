...
export default function Page() {
	const [userId] = useState<string>(() => {
		if (typeof window !== "undefined") {
			return localStorage.getItem("whop_userId") || "user_" + Date.now();
		}
		return "user_" + Date.now();
	});
	const [displayName, setDisplayName] = useLocalStorageState<string>("whop_displayName", "Trader");
	const [trades, setTrades] = useLocalStorageState<Trade[]>(`trades_${userId}`, []);
	const [blockedTickers, setBlockedTickers] = useLocalStorageState<BlockedTicker[]>(`blocklist_${userId}`, []);
	const [dailyCheckIns, setDailyCheckIns] = useLocalStorageState<DailyCheckIn[]>(`checkins_${userId}`, []);
	const [achievements, setAchievements] = useLocalStorageState<Achievement[]>(`achievements_${userId}`, ACHIEVEMENTS);
	...
	const handleExportCSV = () => {
		const headers = ["Date", "Ticker", "Entry", "Exit", "Quantity", "P&L", "Setup", "Emotion", "Followed Rules", "Notes"];
